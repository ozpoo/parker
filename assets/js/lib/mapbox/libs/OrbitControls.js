...
( function () {
	...
	THREE.OrbitControls = function ( object, domElement ) {
		...
		// The four arrow keys
		this.keys = { LEFT: 65, UP: 87, RIGHT: 68, BOTTOM: 83 };//{ LEFT: 37, UP: 38, RIGHT: 39, BOTTOM: 40 };
		...
		this.moveTo = function(coords, currentHeight){
			...
			window.setTimeout(function(){updateTiles(true)}, 10);
		}
		...
		function onMouseDown( event ) {
			...
			if ( event.button === scope.mouseButtons.ORBIT || event.ctrlKey) {
		...
		function onKeyDown( event ) {
			...
			scope.keyDown = window.setInterval(function(){
		...
		function onKeyUp(event){
			window.clearInterval(scope.keyDown);
			scope.keyDown = false;
			stopMap();
		}
		...
		function stopMap(){
			window.clearInterval(scope.moving);
			scope.moving = false;
		}
